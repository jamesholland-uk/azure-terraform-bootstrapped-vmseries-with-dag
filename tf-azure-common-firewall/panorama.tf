...
resource "panos_panorama_address_group" "webservers" {
    name = "webservers-dag"
    description = "Created and Managed By Terraform"
    dynamic_match = "'azure.tag.type.webserver'"
}
...
resource "panos_panorama_security_policy_group" "secrules" {
  ...
  rule {
    ...
    hip_profiles          = ["any"]
    name                  = "Allow Outbound from Web Servers"
    services              = ["any"]
    source_addresses      = [panos_panorama_address_group.webservers.name]
    source_users          = ["any"]
    source_zones          = [panos_panorama_zone.trust.name]
    tags                  = [panos_panorama_administrative_tag.terraform.name]
    log_start             = true
    log_end               = true
    log_setting           = panos_panorama_log_forwarding_profile.logfwd.name
  }
}

resource "panos_panorama_log_forwarding_profile" "logfwd" {
    name = "default"
    device_group = "shared"
    match_list {
        name = "traffic-to-rama"
        log_type = "traffic"
...
resource "null_resource" "panorama_commit" {
  provisioner "local-exec" {
    command    = "tf-azure-common-firewall/scripts/commit.sh ${var.panorama.primary} ${var.panorama.apikey}"
    on_failure = continue
  }
  depends_on = [
    ...
    panos_panorama_security_policy_group.secrules
  ]
}